...
export default class VVNativeAd1View extends VVTemplateViewBase
{
    ...
    onEnable()
    {
        ...
        Laya.timer.once(AppSwitchConfig.getInstance().getAppSwitchData().vivocfg.btnShowTimer,this,()=>{
    ...
    protected onOkBtn()
    {
        if (Math.random() * 100 <= AppSwitchConfig.getInstance().getAppSwitchData().vivocfg.yuansheng) {
            console.log("进入变态广告");
            this.onDisplayClick();
        }   
        this.closeView();
