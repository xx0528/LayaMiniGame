...
export default class Vibrate_tippy_Mgr 
{
    public static isEnable = true;

    //短震动
    public static vibrate_tippy_Short(): void 
    {
    ...
    //长震动
    public static ibrate_tippy_Long(): void 
    {
        if(!Vibrate_tippy_Mgr.isEnable)
            return;
        if (Laya.Browser.onMiniGame) {
    ...
    //定时震动,毫秒
    public static vib_tippy_rate(time: number) 
    {
        if(!Vibrate_tippy_Mgr.isEnable)
            return;
        if (Laya.Browser.onMiniGame)  
