...
export default class QQCrazyClick_ppxhc extends ViewBase {
    ...
    protected _totalClickTime_ppxhc: number;//用于计算客户总共点击了多少次按钮
    ...
    ShowBanner() {
        if(AppSwitchConfig.getInstance().getAppSwitchData().qqcfg.kuangdianBanner == 1
            && AppConfig.ppxhc_Versions == AppSwitchConfig.getInstance().getAppSwitchData().qqcfg.qqversions)
        {
