...
import KRQ_ppxhc_RockSingleAd from "../Com/KRQ_RockSingleAd";

export default class KRQ_Floating extends KRQ_ViewComBase
{
    protected _centerZone : Laya.Clip = null;
    protected _rockSingleAds : Array<KRQ_ppxhc_RockSingleAd> = new Array<KRQ_ppxhc_RockSingleAd>();
    ...
    onAwake()
    {
        ...
        for (let i = 0; i < this._centerZone.numChildren; ++i)  
        {
            ...
            if(child.visible)
            {
                let rockAd = child.getComponent(KRQ_ppxhc_RockSingleAd) as KRQ_ppxhc_RockSingleAd;
