...
export default class MaiLiang {
    ...
    public static uclick: string = "";
    public static stay: string = "";
    ...
    public static GetMaiLiangOpenId(onSuccess: Function, onFail: Function) {
        if (Laya.Browser.onMiniGame) {
            ...
            if (option != null) {
                ...
                if (key != null && key != "" && User_yy.openId != "") {
                    MaiLiang.key = key;
                    let req = new requestData();
                    req.url = MaiLiang.uclick;
                    req.onSuccess = onSuccess;
                    req.onFail = onFail;
                    req.data.appid = App_myqq_Config.AppID;
                    req.data.openid = "";
                    let time = new Date().getTime() / 1000;
                    req.data.posttime = time;
                    req.data.auth = 0;
                    req.data.key = key;
                    req.data.wxopenid = User_yy.openId;
                    req.meth = "POST";
