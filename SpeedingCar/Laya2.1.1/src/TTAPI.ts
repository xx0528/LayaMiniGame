...
export default class TT_wcjtn_API
{
    ...
    public static readonly _templateId_wcjtn_ = ""       //分享素材id
    
    private static record_wcjtn_Res:string = ""
    private static record_wcjtn_:any;

    protected static _banner_wcjtn_ : any = null;    

    public static _wcjtn_ttLogin_wcjtn_(onSuccess: Function, onFail: Function) {
        if (App_wcjtn_Config.onTTMiniGame && null != Laya.Browser.window["tt"]) {
    ...
    protected static on_wcjtn_Rewarded_wcjtn_Video_wcjtn_AdLoad() {
        console.log('激励视频 广告加载完成')
    }
    protected static on_wcjtn_Rewarded_wcjtn_VideoAd_wcjtn_Error(err) {
     ...
     * 开始录屏
     */
    public static start_wcjtn_Record(duration = 300){
        if(!App_wcjtn_Config.onTTMiniGame)return;
     ...
     * 停止录屏
     */
    public static stop_wcjtn_Record() {
        if(!App_wcjtn_Config.onTTMiniGame)return;                     
    ...
    //---------------------分享录屏----------------------------------------
    public static share_wcjtn_Record(callback: Function = null, Failcallback: Function = null) {
        if (!App_wcjtn_Config.onTTMiniGame) return;
        if (TT_wcjtn_API.record_wcjtn_Res != "") {
            window["tt"].shareAppMessage({
                channel: "video",
                extra: {
                    videoPath: TT_wcjtn_API.record_wcjtn_Res, // 可替换成录屏得到的视频地址
                    videoTopics: [App_wcjtn_Config.Game_wcjtn_Name]
                },
                success() {
                    if (callback != null) {
                        callback();
    ...
    //---------------------分享好友----------------------------------------
    public static share_wcjtn_(complate:Function=null){
        if(!App_wcjtn_Config.onTTMiniGame && null == Laya.Browser.window["tt"])return;   
        window["tt"].shareAppMessage({
    ...
    public static show_wcjtn_Banner():any
    {
        if(!App_wcjtn_Config.onTTMiniGame || null == Laya.Browser.window["tt"] || TT_wcjtn_API.banner_wcjtn_AdUnitId.length<=0)return;
        if(!TT_wcjtn_API._banner_wcjtn_){
            ...
            TT_wcjtn_API._banner_wcjtn_.onResize(size => {
                console.log(size.width, size.height);
                TT_wcjtn_API._banner_wcjtn_.style.top = windowHeight - size.height;
                TT_wcjtn_API._banner_wcjtn_.style.left = (windowWidth - size.width) / 2;
            });
    ...
    public static autoOpenSignInView(complate : Function)
    {
        Http_wcjtn_Unit.GetSignIn((res)=>
        {
            ...
            if(isSign == 0)
            {
                View_wcjtn_Mgr.ins_wcjtn_tance.open_wcjtn_View(View_wcjtn_Def.TTSignInView,null,()=>
                {
                    complate(true);
                })
            }
    ...
    //             adUnitId: TT_wcjtn_API.InsAdUnitId,
    //         })
